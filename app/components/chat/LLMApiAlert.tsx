...
export default function LlmErrorAlert({ alert, clearAlert }: Props) {
  ...
  const getErrorMessage = () => {
    switch (errorType) {
      case 'authentication':
        return t('llmError.authFailed', { provider });
      case 'rate_limit':
        return t('llmError.rateLimited', { provider });
      case 'quota':
        return t('llmError.quotaExceeded', { provider });
      default:
        return t('llmError.genericError');
    }
  };
            ...
            >
              ...
              {description && (
                <div className="text-xs text-bolt-elements-textSecondary p-2 bg-bolt-elements-background-depth-3 rounded mt-4 mb-4">
                  {t('alert.errorDetails')} {description}
                </div>
              )}
            </motion.div>
